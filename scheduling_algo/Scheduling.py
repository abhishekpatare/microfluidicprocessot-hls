...
class Module:
    ...
    def storage(self):
        return math.ceil(self.dim[0]*self.dim[1]*self.beta)
...
def _schedule(u, prev_mod, adj_t, mod_tab, Na, max_t, M, S, next, t_curr):
    ...
    while len(q)>0:
        ...
        while t>=0:    
            ...
            if curr_mod.area() + max(m_window)<=Na and (len(s_window)==0 or curr_mod.storage() + max(s_window)<=Na):
                done = True
                break
    ...
    for u in crit_path:
        ...
        while tr-tl>1:
            ...
            if possible:
                ...
                tl=tm
            scheduled_critpath_u  = scheduled_critpath_u or possible
        
        scheduled_all = scheduled_all or scheduled_critpath_u
